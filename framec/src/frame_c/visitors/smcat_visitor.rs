...
impl SmcatVisitor {
    ...
    pub fn new(   arcanium:Arcanum
                  ...
                  , _comments:Vec<Token>) -> SmcatVisitor {
        ...
        SmcatVisitor {
            ...
            _generate_exit_args: generate_exit_args,
            states:String::new(),
    ...
    fn generate_states(&self, node_name:&String, is_system_node:bool, indent:usize, output:&mut String) {
        ...
        if has_children {
            match output.rfind(",") {
            Some(location) => output.replace_range(location..location+1, ";"),
            None => ()
            };
        }
    ...
    fn generate_state_ref_change_state(&mut self, change_state_stmt_node: &ChangeStateStatementNode) {
        ...
        let transition_code = &format!("{} => {} [color=\"grey\"] : {}\n"
                                       ,current_state
    ...
    fn generate_state_stack_pop_transition(&mut self, _transition_statement: &TransitionStatementNode) {
   }
}
...
impl AstVisitor for SmcatVisitor {
    ...
    fn visit_system_node(&mut self, system_node: &SystemNode) -> AstVisitorReturnType {
        ...
        match (&system_node).get_first_state() {
            Some(x) => {
                ...
                self.add_code(&format!("initial,\n"));
                self.transitions.push_str(&format!("initial => \"{}\";\n", self.first_state_name));
            },
            None => {},
        }
    ...
    fn visit_interface_block_node(&mut self, _interface_block_node: &InterfaceBlockNode) -> AstVisitorReturnType {
        AstVisitorReturnType::InterfaceBlockNode {}
    }

    //* --------------------------------------------------------------------- *//

    fn visit_interface_method_node(&mut self, _interface_method_node: &InterfaceMethodNode) -> AstVisitorReturnType {
    ...
    fn visit_actions_block_node(&mut self, _actions_block_node: &ActionsBlockNode) -> AstVisitorReturnType {
    ...
    fn visit_domain_block_node(&mut self, _domain_block_node: &DomainBlockNode) -> AstVisitorReturnType {
    ...
    fn visit_number_match_test_pattern_node(&mut self, _match_pattern_node:&NumberMatchTestPatternNode) -> AstVisitorReturnType {
...
//
    ...
    fn visit_frame_event_part(&mut self, _frame_event_part:&FrameEventPart) -> AstVisitorReturnType {
    ...
    fn visit_variable_decl_node(&mut self, _variable_decl_node: &VariableDeclNode) -> AstVisitorReturnType {
    ...
    fn visit_assignment_expr_node(&mut self, _assignment_expr_node: &AssignmentExprNode) -> AstVisitorReturnType {
        AstVisitorReturnType::AssignmentExprNode {}
    }
